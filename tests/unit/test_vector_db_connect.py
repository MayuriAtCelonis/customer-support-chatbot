...
import ai.retrieval.vector_db_connect as vdbc

class TestVectorDBConnect:
    def test_get_sentence_transformer_caches_model(self):
        with patch('ai.retrieval.vector_db_connect.SentenceTransformer') as MockModel:
    ...
    def test_get_qdrant_client_caches_client(self):
        with patch('ai.retrieval.vector_db_connect.SentenceTransformer') as MockModel, \
            patch('ai.retrieval.vector_db_connect.initialize_qdrant') as mock_init_qdrant:
    ...
    def test_init_vector_db_connections_returns_client_and_model(self):
        with patch('ai.retrieval.vector_db_connect.get_qdrant_client') as mock_get_client, \
             patch('ai.retrieval.vector_db_connect.get_sentence_transformer') as mock_get_model:
            mock_get_client.return_value = 'mock_client'
            mock_get_model.return_value = 'mock_model'
            client, model = vdbc.init_vector_db_connections('test_collection', 'mock-model')
